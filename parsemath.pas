...
{Procedure ExprBis( var Result: TFPExpressionResult; Const Args: TExprParameterArray);
var Cerrado:TCerrados ;

begin
     Cerrado:=Cerrado.create;
     A:=ArgToFloat( Args[ 1 ] );
     B:=ArgToFloat(Args[2]);
     ...
     f:=Args[0].ResString;
     i:=1;
     Cerrado.MetodoType:=0;
     error2:=20;
     signo:=-1;
     anterior:=0;
     Result.resFloat:=Cerrado.ejecutarCerrados(A,B,Error,f);
...
begin
   with FParser.Identifiers do begin
       ...
       //AddFunction('biseccion', 'F', 'SFFF', @ExprBis);//two arguments 'FF'
